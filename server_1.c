...
void send_active_client(int sockfd) {
    ...
    for(int i=0 ; i< ClientsNumber ; i++) {
        if(client[i]) {
            ...
            sprintf( s, "<< [%d] %s\r\n",client[i]->user_id, client[i]->name); // client er msg er somoy id r name dekhay, line 272
...
void *handle_client( void *arg) {
    ...
    printf("<< accept ");
             // client request accept er print out copy
    print_client_addr(cli->address); //printf("loop running: %s\n", cli->sockfd);
    printf(" referenced by user_id %d\n", cli->user_id);

    read(cli->sockfd,buff_in,sizeof(buff_in)-1);

    sprintf( buff_out,"%s <<(client name) has joined\r\n", buff_in);
    ...
    while(( rlen =read(cli->sockfd,buff_in,sizeof(buff_in)-1) ) > 0) {
        ...
        if(buff_in[start_index_of_messege]=='/') {
            ...
            command = strtok( client_messege, " ");  //modified version--strtok used for splitting the message Splits str[] according to given delimiters.
            ...
            if(!strcmp( command, "/quit")) {
                ...
                if(param) {
                    ...
                    sprintf(buff_out, "<<%s is now known as %s\r\n", old_name, cli->name); // 2 ta kore arrow use korsi
                ...
                if(param) {
                    ...
                    if(param) {
                        ...
                        send_message_client( buff_out, uid);   /* particular client ke msg dicche that means private msg */
                    }
                    ...
                    send_message_self("<< reference cannot be null(reference should be client id)\r\n", cli->sockfd);
                }
                ...
                strcat(buff_out, "<< /msg      <reference(client id)> <message> Send private message\r\n");
                strcat(buff_out, "<< /list     Show active clients\r\n");
                strcat(buff_out, "<</file      <send file>\r\n")
